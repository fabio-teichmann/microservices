...
func emailEntryFromRow(row *sql.Rows) (*EmailEntry, error) {
	...
	var optOut bool
	// scans in same order of table columns
...
func GetEmail(db *sql.DB, email string) (*EmailEntry, error) {
	// Query keeps connection open until closed !
	rows, err := db.Query(`
...
func UpdateEmail(db *sql.DB, entry EmailEntry) error {
	...
	// Upsert operation (tries insert, if already exists: update existing entry)
	_, err := db.Exec(`
		INSERT INTO emails(email, confirmed_at, opt_out)
		VALUES(?, ?, ?)
...
// create structure to allow for pagination of results
...
func GetEmailBatch(db *sql.DB, params GetEmailBatchQueryParams) ([]EmailEntry, error) {
	...
	// only need to close once we started reading from rows
	defer rows.Close()
